...
import com.kauel.pinchazoSmart.databinding.FragmentVolumenBinding
...
//            }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            binding.apply {
                val data1 = SendDataVolumen(
                    id_phone =  idPhone,
                    height = stringToDouble(edtHigh.text.toString()),
                    length = stringToDouble(edtLong.text.toString()),
                    width = stringToDouble(edtWidth.text.toString()),
                    volume = stringToDouble(edtVolume.text.toString()),
                    rut = tvRutPortico.text.toString(),
                    date = date,
                    gate = tvPortico.text.toString(),
    ...
    private fun showSuccessView(data: Volumen?) {
        ...
        val pathImage = ip + URL_IMAGE + nameImage
    ...
    private fun showErrorView(error: Throwable?) {
        binding.apply {
            customProgressDialog.dismiss()
            view?.makeSnackbar(error?.localizedMessage.toString(), false)
        }
    }
